...
void FileParser::run()
{
	...
	while(!m_bExit)
	{
		...
		for(int i=0; i<len; ++i)
		{
			...
			//每次不管能否正确解析文件，之后要清掉96点功率值vector<>和n个96点功率值的vector<vector<>>
...
bool FileParser::GetFiles(string path)  
{  	
	...
	while(dir_iteratorWai.hasNext())
	{
		...
		m_vecFiles.push_back(absolute_file_path.toStdString());
		bSuc = true;
	}
	sort(m_vecFiles.begin(), m_vecFiles.end());//排序
	return bSuc;
} 
...
bool FileParser::ReadInConfigPara()
{
	...
	while(!t.atEnd())
	{
		...
		unsigned int nEqualSignPos = str.find('=');
		if(nEqualSignPos != string::npos) //如果有等号，说明有配置项，准备读取
		{
			...
			/*unsigned*/ int nChnCommaPos = str.find("，"); //clw note：如果是unsigned，这里结果在linux和windows都是429496725；所以个人感觉应该不能写成无符号，应该写成int然后结果-1？
			/*unsigned*/ int nEngCommaPos = str.find(',');
			   ...
			   nChnCommaPos == string::npos && nEngCommaPos != string::npos)
			{
				...
				printf("clw：既有中文逗号，又有英文逗号！输入格式不合理！\n");
			bSuc = true;
		}
	}
...
//根据文件路径获取文件名
string FileParser::GetFileNameByPath(string filepath)
{
...
bool FileParser::ParseESFile(const char filepath[])
{
	...
	unsigned int nSuffixPos = fileName.rfind('.'); //如果确定文件名只有一个.字符，那么这样写就行
	//unsigned int nSuffixPos = fileName.rfind(".txt");
	//if(nSuffixPos == string::npos) //如果找不到.txt后缀，再尝试.TXT后缀
	//	nSuffixPos = fileName.rfind(".TXT");

	unsigned int nRightUnderLinePos = fileName.rfind('_'); //准备取最后一个_到.TXT中间的内容
	string fileType = "";
	if(nSuffixPos != string::npos && nRightUnderLinePos != string::npos)
	{
		...
		if(strcmp(fileType.c_str(), FILETYPE_ULTRALOAD) == 0 || strcmp(fileType.c_str(), FILETYPE_AGCRN) == 0) 
		{
		...
		else if(strcmp(fileType.c_str(), FILETYPE_SHORTLOAD) == 0 || strcmp(fileType.c_str(), FILETYPE_AGCRQ) == 0)
		{
	...
	for(int i = 0; i < fileLen; ++i)
	{
		...
		{
			bFindFileName = true;
			m_nFilesInfoIndex = i; //记录该文件对应的文件结构体索引号，方便以后使用
			break;
		}		
	}
	if(!bFindFileName)
	...
	for(map<string, vector<string> >::iterator iter = m_vecFilesInfo[m_nFilesInfoIndex].m_map_nameVals.begin(); 
		iter !=  m_vecFilesInfo[m_nFilesInfoIndex].m_map_nameVals.end(); ++iter)
	{
		char strRecordID[BUF_LEN_MIN] = {0};
		if(fileType == FILETYPE_SHORTLOAD || fileType == FILETYPE_ULTRALOAD)
			strncpy(strRecordID, iter->second[2].c_str(), BUF_LEN_MIN - 1);//如#3，在这里只保留3
		else if(fileType == FILETYPE_AGCRN || fileType == FILETYPE_AGCRQ)
		...
		//方法一：
		/*
		char* pAtSign = strstr(pTxtBuf, "@");
		if(pAtSign == NULL)
		{
			//error1
			printf("clw：TXT中找不到@，请检查！\n");
			free(pTxtBuf);
			fclose(fp);
			return false;
		}
		char* pSpace = strstr(pAtSign, " ");
		char* pTab = strstr(pAtSign, "\t");
		char strRecordIDCombine[BUF_LEN_MIN] = {0}; //记录了TXT中实际的格式，如"# 3"

		//pSpace - pAtSign == 1说明@id后面有空白，那么后面记录也就是# 10的格式，
		//也会有空白。这里暂时只考虑1、一个空格 2、两个空格 3、一个Tab三种情况
		if(pSpace - pAtSign == 1) 
		{
			if(strstr(pTxtBuf, "# ") == NULL)
			{
				if(strstr(pTxtBuf, "#  ") == NULL)
				{
					//一般不会这么坑吧....
					printf("clw：#后面多于2个空格，请检查格式！\n");
				}
				else
				{
					sprintf(strRecordIDCombine, "%s%d", "#  ", nRecordID);
				}
			}
			else
			{
		...
		char* strRecordIDCombine;
		char* pBegin = strstr(pTxtBuf, strRecordIDCombine); //指针指向#id最前面，然后对该行进行操作
		if(pBegin == NULL)
		{
			//error2
			printf("clw：TXT找不到#id的记录！\n");
			free(pTxtBuf);
			fclose(fp);
			return false;
		}
		*/
		...
		while(1) 
		{
			...
			if(/*isdigit(*(pPre + 1))*/ (*(pPre + 1) >= DIGIT_ASCII_MIN && *(pPre + 1) <= DIGIT_ASCII_MAX || *(pPre + 1) == '-'))  
			{
				...
				if(strcmp(fileType.c_str(), FILETYPE_SHORTLOAD) == 0 || strcmp(fileType.c_str(), FILETYPE_AGCRQ) == 0)
				{
...
bool FileParser::WriteFileInfoToDB()
{
	...
	string fileType = m_vecFilesInfo[m_nFilesInfoIndex].m_map_nameInfo["filetype"];
	...
	{
		strSqlHeader = "INSERT INTO ultraloadhisdata_" + curYear + "(ID, CTRLID, FORCASTTIME, SAVETIME, METHOD, FORCASTP) VALUES";
	}
	else if(fileType == FILETYPE_SHORTLOAD)
	{
		strSqlHeader = "INSERT INTO shortloadhisdata_" + curYear + "(ID, CTRLID, FORCASTTIME, SAVETIME, METHOD, FORCASTP) VALUES";
	} //TODO：其他类型文件，后续补充；每种类型的时间间隔还需要修改，比如5分钟，15分钟等，点数有48点和96点；
	else if(fileType == FILETYPE_AGCRN)
	{
		strSqlHeader = "INSERT INTO stagctodayplan_" + curYear + "(ID, PLANDATE, PLANVALUESQ, AUTOPLANAIM, PLANREVISEDAIM, OPRTREVISEDAIM) VALUES";
	}
	else if(fileType == FILETYPE_AGCRQ)
	{
		strSqlHeader = "INSERT INTO stagcnextdayplan_" + curYear + "(ID, PLANDATE, PLANVALUESQ, AUTOPLANAIM, PLANREVISEDAIM, OPRTREVISEDAIM) VALUES";
	}

	QString sqlToExec = strSqlHeader;
	char strSQL[1024];
	QStringList lstSql;
	...
	if(fileType == FILETYPE_SHORTLOAD || fileType == FILETYPE_AGCRQ)
	{
		//forcastTime = curTime.addDays(1);//当前时间+1天，之后会用到这个年月日
		m_strForcastTime.append(" 00:00:00");
	}
	else if(fileType == FILETYPE_ULTRALOAD || fileType == FILETYPE_AGCRN)
	{
	...
	*/
			...
			iter !=  m_vecFilesInfo[m_nFilesInfoIndex].m_map_nameVals.end(); ++iter)
	{
		if(fileType == FILETYPE_SHORTLOAD || fileType == FILETYPE_ULTRALOAD)
		{
			strFormat = "(%d, %d, '%s', '%s', %d, %f)";
			strID = iter->second[0];
			nID = atoi(strID.c_str());
			strCtrlID = iter->second[1];
			nCtrlID = atoi(strID.c_str());

			for(int i = 0; i < 96 /*96，1测试用*/; ++i)
			{
				sprintf(strSQL, strFormat, nID,  nCtrlID , vecForcastTime[i].c_str(), curDateTime.toLatin1().data(), 0/*method，暂时写0*/, m_vecStationShortPs[count][i]);
